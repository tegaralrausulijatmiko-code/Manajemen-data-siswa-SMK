...
<div class="page-header">
    <div>
        ...
        <div class="breadcrumb">
            ...
            <a href="<?= base_url('admin/absensi/mapel') ?>">Absensi Mapel</a> /
            <?= esc($jadwal['nama_mapel'] ?? 'Mapel') ?>
        </div>
    </div>
    <a href="<?= base_url('admin/absensi/mapel?tanggal=' . $tanggal) ?>" class="btn btn-secondary btn-sm">
...
<div class="card">
    ...
    <div style="padding:8px 20px; background:var(--bg-secondary); border-bottom:1px solid var(--border);">
        <span style="font-size:0.82rem; color:var(--text-muted);">
            Tanggal absensi: <strong><?= date('d M Y', strtotime($tanggal)) ?></strong>
        </span>
    </div>

    <form method="post" action="<?= base_url('admin/absensi/mapel/' . $jadwal['id_jadwal'] . '/simpan') ?>">
        <?= csrf_field() ?>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Nama Siswa</th>
                        <th width="400">Status Kehadiran</th>
        ...
        <?php if (! empty($siswa_list)): ?>
            <div style="padding:16px 20px; border-top:1px solid var(--border); display:flex; justify-content:flex-end; gap:10px;">
                <a href="<?= base_url('admin/absensi/mapel?tanggal=' . $tanggal) ?>" class="btn btn-secondary">Batal</a>
