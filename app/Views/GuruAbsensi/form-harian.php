...
<div class="stats-grid" style="margin-bottom:20px;">
    ...
    <div class="stat-card">
        <div class="stat-icon" style="background:#f0fdf4; color:#16a34a;"><i class="ri-group-line"></i></div>
        <div class="stat-text">
            <p>Jumlah Siswa</p>
            <h4><?= count($siswa_list) ?> siswa</h4>
        </div>
    </div>
</div>
...
<div class="card">
    ...
    <div style="padding:12px 20px; background:var(--bg-secondary); border-bottom:1px solid var(--border); display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
        <span style="font-size:0.82rem; color:var(--text-muted); margin-right:4px;">Tandai semua:</span>
        <?php foreach ($status_list as $st):
            $btnStyle = [
                'Hadir' => 'background:#16a34a; color:#fff;',
                'Izin'  => 'background:#d97706; color:#fff;',
                'Sakit' => 'background:#2563eb; color:#fff;',
                'Alpha'  => 'background:#dc2626; color:#fff;',
            ][$st] ?? '';
        ?>
            <button type="button" class="btn btn-sm quick-all" data-status="<?= esc($st) ?>"
                    style="<?= $btnStyle ?>">
                <?= esc($st) ?>
            </button>
        <?php endforeach; ?>
    ...
    <form method="post" action="<?= base_url('guru/absensi/harian/' . $kelas['id_kelas'] . '/simpan') ?>" id="form-harian">
        <?= csrf_field() ?>
...
echo view('Template/layout', [
    ...
    'subtitle'  => 'Presensi harian wali kelas',
    'content'   => $content,
    'extra_css' => $extra_css,
    'extra_js'  => $extra_js,
]);
